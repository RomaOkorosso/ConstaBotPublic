...
def request_battle(bot, message, count):
    try:
        if message.reply_to_message is not None:
            ...
            bot.reply_to(
                message,
                "Необходимо отправить команду /fight ответом (реплай)"
        ...
        if name1 is not None and name2 is not None:
            bot.reply_to(
                ...
                f"Игрок {name1[0]} вызывает игрока {name2[0]}"
                f" на дуэль (🏹{count} раз). Будем биться?",
                reply_markup=markup,
            )
        elif name1 is None:
            bot.reply_to(
                message,
                "Вас нет в моем списке игроков, необходимо скинуть свой профиль из игры",
            )
        elif name2 is None:
            bot.reply_to(
                message, "Противника нет у меня в списках, наобходимо скинуть профиль"
            )
    except:
        pass
...
def cancel(bot, query):
    ...
    bot.edit_message_text(
        "Соперник отказался от дуэли 😒", query.message.chat.id, query.message.message_id
    )
...
def accepted(bot, query):
    try:
        ...
        if count < 5:
            fight_txt = (
                f"Сражение между 🏅{first[6]}{first[0]}({first[1]}❤️) 🏅{second[6]}{second[0]}"
                f"({second[1]}❤️)\n\n"
            )
            for i in range(count):
                ...
                if count == 1:
                    bot.edit_message_text(
                        fight_txt, query.message.chat.id, query.message.message_id
                    )
