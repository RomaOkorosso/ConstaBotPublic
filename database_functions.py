...
def alarm(bot, message, error):
    bot.send_message(
        message.chat.id, "Случилась ошибка, разработчик получил информацию"
    )
    error_msg = (
        str(error)
        + "\n\n"
        ...
        if bd_arr is not None:
            for num in range(count):
                if icon_arr[num] in s:
                    ...
                    if len(s) - len(str_arr[0]) <= 2:
                        ...
                        val_arr[num] = str_tmp[str_tmp.find("+") + 1 :]
                    if icon_arr[num] in ["⏳", "💨", "🔮", "🎯"]:
                        if round(float(bd_arr[num]), 2) >= round(
                            float(val_arr[num]), 2
                        ):
                            delta = round(
                                round(float(val_arr[num]), 2)
                                - round(float(bd_arr[num]), 2),
                                2,
                            )
                            ...
                            if icon_arr[num] == "💨":
                                postgres_insert_query = (
                                    """ update armor SET agi = %s where name = %s"""
                                )
                            elif icon_arr[num] == "🔮":
                                postgres_insert_query = (
                                    """ update armor SET exp = %s where name = %s"""
                                )
                            elif icon_arr[num] == "🎯":
                                postgres_insert_query = (
                                    """ update armor SET cri = %s where name = %s"""
                                )
                            elif icon_arr[num] == "⏳":
                                postgres_insert_query = (
                                    """ update armor SET accu = %s where name = %s"""
                                )
                        ...
                        if int(bd_arr[num]) >= int(val_arr[num]):
                            ...
                            if icon_arr[num] == "❤":
                                postgres_insert_query = (
                                    """ update armor SET hp = %s where name = %s"""
                                )
                            elif icon_arr[num] == "🛡":
                                postgres_insert_query = (
                                    """ update armor SET def = %s where name = %s"""
                                )
                            elif icon_arr[num] == "⚔":
                                postgres_insert_query = (
                                    """ update armor SET atk = %s where name = %s"""
                                )
            ...
            for i in range(len(icon_arr)):
                postgres_insert_row_name_query += (
                    f", {stat_name[ic_arr.index(icon_arr[i])]}"
                )
...
def get_recipe(message, bot, find_name):
    ...
    # # cur.close()
    return_msg = (
        "Максимальные статы для"
        + recipes_arr[0][
            str(recipes_arr[0]).find("Рецепт")
            + len("Рецепт")
            + 1 : len(str(recipes_arr[0]))
        ]
        + ":\n"
    )
    if recipes_arr is not None:
        for i in range(len(icon_arr)):
            if recipes_arr[i + 2] != 0:
                return_msg = (
                    return_msg + str(icon_arr[i]) + str(recipes_arr[i + 2]) + "\n"
                )
...
def ping_command(message, bot):
    try:
        ...
        cur = con.cursor()
        cur.execute(
            """ SELECT chat_id FROM chat_id_list WHERE chat_id = '{0}'""".format(
                str(message.chat.id)
            )
        )
        bd_arr = cur.fetchone()
        ...
        if bd_arr is None:
            ...
            cur = con.cursor()
            cur.execute(
                """ SELECT login FROM ping_player WHERE chat_id = '{0}'""".format(
                    str(message.chat.id)
                )
            )
            bd_arr = cur.fetchall()
    ...
    try:
        ...
        if bd_arr is None and message.from_user.id != 374085219:
            ...
            postgres_select_query = (
                """ SELECT * FROM ping_player WHERE chat_id = %s and login = %s"""
            )
            ...
            if bd_arr is None:
                cur = con.cursor()
                postgres_insert_query = (
                    """ INSERT INTO ping_player (chat_id, login) VALUES (%s,%s)"""
                )
...
def del_command(message, bot):
    try:
        ...
        if bd_arr is None:
            ...
            if bd_arr is None:
                bot.reply_to(
                    message, "Тебя и так нет в списке /ping_all в данном чате!"
                )
            else:
                cur = con.cursor()
                postgres_del_query = (
                    """ DELETE FROM ping_player WHERE chat_id = %s and login = %s"""
                )
...
def add_chat_command(message, bot, chat_for_adding):
    try:
        ...
        if bd_arr is None:
            cur = con.cursor()
            cur.execute(
                """ INSERT INTO chat_id_list (chat_id) VALUES ('{0}')""".format(
                    str(chat_for_adding)
                )
            )
            con.commit()
            # # cur.close()
            bot.reply_to(
                message, "Чат добавлен в список разрешенных к использованию ConstaBot!"
            )
        else:
            bot.reply_to(
                message,
                "Чат уже давно добавлен в список разрешенных к использованию ConstaBot!",
            )
    except Exception as error:
        alarm(bot, message, error)
...
def del_chat_command(message, bot, chat_id_for_delete):
    ...
    cur = con.cursor()
    cur.execute(
        """ DELETE FROM chat_id_list WHERE chat_id = '{0}'""".format(
            str(chat_id_for_delete)
        )
    )
    con.commit()
    # # cur.close()
    bot.send_message(
        message.chat.id,
        str(chat_id_for_delete) + " ID чата удален из списка рарешенных",
    )
...
def send_totem_info(message, bot, totem_name, lvl, aden, bronze, silver, gold):
    try:
        ...
        bot.reply_to(
            message,
            "Тотем обновлен, сообщение можно удалять. "
...
def add_totem(lvl, aden, bronze, silver, gold):
    try:
        ...
        cur = con.cursor()
        cur.execute(
            """SELECT * FROM required_totems WHERE totem_lvl = %s""", (str(lvl),)
        )
        ...
        if db_array is not None:
            ...
            cur = con.cursor()
            cur.execute(
                """UPDATE required_totems SET totem_aden = %s, totem_bronze = %s,
             totem_silver = %s, totem_gold = %s WHERE totem_lvl = %s""",
                (
                    aden,
                    bronze,
                    silver,
            ...
            cur = con.cursor()
            cur.execute(
                """INSERT INTO required_totems (totem_lvl, totem_aden, totem_bronze,
             totem_silver, totem_gold) VALUES (%s, %s, %s, %s, %s)""",
                (
                    lvl,
                    aden,
                    bronze,
...
def totems_main(bot, message):
    try:
        ...
        cur = con.cursor()
        cur.execute(
            f"""SELECT * FROM players_totems WHERE user_id = '{message.from_user.id}'"""
        )
        db_array = cur.fetchone()
        con.commit()
        # # con.close()
        ...
        if db_array is not None:
            full = [0] * len(items)
            full_msg = (
                f"Информация по тотемам для игрока @{message.from_user.username}\n"
            )
            for i in range(len(x)):
                if db_array[x[i]] is not None and db_array[x[i]] != 10:
                    ...
                    cur = con.cursor()
                    cur.execute(
                        """SELECT * FROM required_totems WHERE totem_lvl = %s""",
...
def get_update_interval(user_id):
    try:
        ...
        cur = con.cursor()
        cur.execute(
            f"""SELECT last_update_time FROM profiles WHERE user_id = {user_id}"""
        )
        db = cur.fetchone()
        cur.execute("""SELECT now()::timestamp""")
        last_date = datetime.datetime(
            db[0].year, db[0].month, db[0].day, db[0].hour, db[0].minute, db[0].second
        )
        now = cur.fetchone()
...
def save_profiles(
    ...
    try:
        ...
        cur = con.cursor()
        cur.execute(
            f"""SELECT user_id, username, hp, lvl, pvp, exp, credits, aden, atk, def,
        ...
        # atk: int, defense: int, cri: float, ddg: float, accu: float
            ...
            updt += (
                f"prem = {prem}, prem_duration = '{prem_duration}', abu = {abu}, abu_duration = "
                f"'{abu_duration}',"
...
def edit_recipe(bot, message, name_res):
    try:
        ...
        if bd_arr is None:
            ...
            for item in users:
                bot.send_message(
                    item, f"Обновлен рецепт {name_res}, я очень надеюсь, что все ок"
                )
...
def add_collumn(bot, message):
    ...
    con = None
    second = [
        """ALTER COLUMN atk_lvl DROP NOT NULL;""",
...
def get_profile_by_id(id):
    try:
        ...
        cur = con.cursor()
        cur.execute(
            f"""SELECT username, hp, atk, def, cri, ddg, lvl, win, loose,
...
def get_mob_by_lvl(lvl, type):
    if type is None:
        ...
        cur = con.cursor()
        cur.execute(
            f"""SELECT mobname, hp, atk, def, cri, ddg, lvl,
...
def parser(bot, message):
    ...
    while txt.find("🏅", left, len(txt)) != -1:
        ...
        if db_array is None:
            cur.execute(
                f"""INSERT INTO mobs (mobname, hp, lvl, atk, def, cri, ddg, accu) VALUES ('{mobname}', {hp},
...
def get_list_of_players_bu_nickname(players: list):
    ...
    for i in range(len(players)):
        ...
        cur.execute(
            f"""SELECT user_id FROM profiles WHERE username LIKE '{players[i]}'"""
        )
        now_id = cur.fetchone()
...
def add_caves(txt: str):
    try:
        ...
        cur.execute(
            f"""INSERT INTO caves (lider_name, entry_datatime) VALUES ('{lider_name}', '{entry_datatime}');"""
        )
        con.commit()
    except Exception as error:
        print(error)
...
def set_shield(txt: str):
    try:
        ...
        cur.execute(
            f"""UPDATE caves SET end_shield_time = now() WHERE lider_name = '{first_lider}'"""
        )
        cur.execute(
            f"""UPDATE caves SET end_shield_time = now() WHERE lider_name = '{second_lider}'"""
        )
        con.commit()
    except:
        pass
    ...
    try:
        ...
        if len(res_old) == 0:
            for i in range(len(res[0])):
                cur.execute(
                    f"""INSERT INTO resources (user_id, res_name, res_count) VALUES ({user_id}, '{res[0][i]}',
                                {res[1][i]});"""
                )
        else:
