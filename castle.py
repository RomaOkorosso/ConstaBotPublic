    ...
    try:
        ...
        if chat_info.pinned_message is not None:
            if chat_info.pinned_message.from_user.is_bot is True:
                ...
                pinned_castle = pin_txt[
                    pin_txt.find("на битву в замок ")
                    + len("на битву в замок ") : pin_txt.find("\nПрисылайте")
                ]
            ...
            if "Ты направляешься в замок" in txt:
                to_castle = txt[
                    txt.find("Ты направляешься в замок ")
                ...
                if gamename in pin_txt:
                    ...
                    if now_icon == "🐾":
                        new_msg = (
                            pin_txt[0:left] + "🌚" + pin_txt[pin_txt.find(" ", left) : :]
                        )
                        # message.text = new_msg
                        bot.delete_message(message.chat.id, message.message_id)
                        bot.edit_message_text(
                            new_msg, message.chat.id, pinned_castle_msg_id
                        )
    ...
    try:
        ...
        if first in pin_txt:
            if "Ты одержал победу над " in txt:
                left = txt.find("Ты одержал победу над ") + len(
                    "Ты одержал победу над "
                )
            ...
            first_w_l = pin_txt[
                left_f + len(first) + len(" 🌚 бои: ") : pin_txt.find("\n", left_f)
            ].split("/")
            left_s = pin_txt.find(second)
            if pin_txt.find("\n", left_s) != -1:
                second_w_l = pin_txt[
                    left_s + len(second) + len(" бои: ") : pin_txt.find("\n", left_s)
                ].split("/")
            else:
            ...
            bot.send_message(
                message.chat.id, f"Ты ТОЧНО вышел в замок {pinned_castle}?"
            )
    except:
        pass
