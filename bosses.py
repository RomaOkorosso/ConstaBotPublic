...
def boss_starter(bot, message):
    try:
        ...
        bot.reply_to(message.reply_to_message, f'{name}Выберите босса на которого будет собран рейд', markup=markup)
...
def mob_fight(bot, message, mob_lvl, player_id, mob_type):
    ...
    if second is None:
        bot.send_message(message.chat.id, "Необходимо ввести сообщение в формате /mobf <lvl> <e> Этер опционален")
        return 0
    ...
    while first[1] > 0 and second[1] > 0 and exit_fl == 0:
        if second[5] > round(random.uniform(0.0, 100.0), 2) and exit_fl == 0:
            fight_txt += f'{second[0]}  увернулся от атаки 💨\n'
                ...
                first[2] - second[3]) >= 310 and exit_fl == 0:
        ...
        if first[5] > round(random.uniform(0.0, 100.0), 2) and exit_fl == 0:
            fight_txt += f'{first[0]}  увернулся от атаки 💨\n'
                ...
                second[2] - first[3]) >= 310 and exit_fl == 0:
...
def mob_fight_more_1_without_heal(bot, message, mob_lvl, player_id, mob_type, count: int = 10000):
    try:
        first = list(database_functions.get_profile_by_id(player_id))
        if first is not None:
            ...
            for i in range(count):
                ...
                if first[1] > 0 and first[0] == player:
                    ...
                    lll += 1
                    # bot.send_message(message.chat.id, 'win')
                elif (first[1] > 0 and first[0] != player) or (second[1] > 0 and second[0] != player):
                    if first[0] != player:
                        first, second = second, first
            ...
            first[11] = ll[0]
            fight_txt += f'{first[0]} максимальная серия побед над мобом {second[0]}: {first[11]}\nВ среднем:' \
